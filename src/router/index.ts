...
router.beforeEach((to, from, next) => {
    ...
    if (whiteListRoutes.indexOf(to.path) != -1) {
        // 只有这些页面才允许不登陆的情况下访问
        next();
    } else {
        if (token) {
                ...
                axios
                    ...
                    .then((response) => {
                        ...
                        if (result.code == 0) {
                            // token有效
                            if (to.path == "/login") {
                                // 访问的是登录页面就跳到Home，可以开始正常使用国建安全培训各项功能
                                next({ path: "/home" });
                            } else {
                                // 别的页面随便访问，因为已经登录并且token有效
                                ...
                                // 动态添加路由
                                result.data["sidebarMenuList"].forEach(
                                    (menuItem: Menu) => {
